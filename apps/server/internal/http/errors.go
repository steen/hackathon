...
// Stable error codes used across the auth surface. Kept as a small
// enum so clients can branch on Error.Code rather than parse Message.
const (
...
// WriteOKStatus is the variant of WriteOK that lets the caller pick a
// non-200 success status (e.g. 201 Created from POST /api/register).
func WriteOKStatus(w http.ResponseWriter, status int, data interface{}) {
