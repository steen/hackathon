...
`feature-ts-api-client-package` (PR #67, with prettier follow-up #f85f955) ships `packages/api-client/` as `@hackathon/api-client` in the pnpm workspace. `Client` + `WebSocketClient` (with reconnect + `setTimeout` injection for deterministic tests) + `watch` async-iterable; 25 vitest cases. AC-6 (presence-event surface) sits at partial because the wire format disagrees with the now-real server emission — see schema-drift flag below. AC-7 (consumable by apps/web) is now load-bearing-real: `apps/web/package.json:15` declares `"@hackathon/api-client": "workspace:*"` and the web's vitest suite exercises the import resolution.

`feature-presence` (PR #80, server-side only) ships the hub-level refcount + WS broadcast + `GET /api/presence` REST endpoint. 17 in-package tests across `hub/presence_test.go` (9), `wsapi/presence_test.go` (4), `http/presence_handlers_test.go` (4). AC-4 (web app + CLI consumers) deferred until those clients consume it — the web app's chat page has an explicit TODO placeholder pending api-client wrapping `GET /api/presence` (see `web-app.md` AC-5 partial).

`feature-web-app` (PR #84, with follow-up `c8c80ad`) ships `apps/web/` as a Vite + React + TypeScript app with `Login`, `Register`, `Chat` routes consuming `@hackathon/api-client`. 6 vitest cases covering all 5 named tests from the spec's Test plan: login, register-with-invite-code, chat history+live-append, reconnect-mints-fresh-ticket, and the SEC-9 XSS test (literal `<script>` body renders as text). AC-5 (presence list) at partial — chat page has an explicit TODO acknowledging the api-client+presence-schema-drift gap.
