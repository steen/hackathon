...
| AC-4 | The TODO at `apps/server/internal/wsapi/handler.go:148` (`_ = userID`) is removed. | deferred | impl is stub — confirmed `_ = userID` still present (3-line offset from the spec's 148 reference at the analyzed SHA; grep `_ = userID` if line numbers drift further). |
