...
- **`feature-auth-internals` AC-5 is now closed.** That AC was previously partial (PR #47): "signing key is loaded from config". `apps/server/main.go` now wires `cfg.JWTSecret` (validated by `feature-startup-config-checks`) into `httpapi.NewAuthHandlers(... SigningKey: []byte(cfg.JWTSecret) ...)`. The full chain is: env → `config.Load` → `Validate` → main → `NewAuthHandlers.SigningKey` → `auth.Issue` / `auth.Parse`. **Recommendation:** the next test-watch tick should re-promote auth-internals AC-5 from `partial` to `covered` and surface the full plumbing path in those findings.
