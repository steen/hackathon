...
- **What to assert:** Boot the server. Dial `ws://127.0.0.1:<port>/ws` with `coder/websocket.Dial`. Write a single text frame of exactly 64*1024 bytes → connection stays open (assert subsequent `Read` blocks/times out, not closes). Dial fresh; write a single text frame of 64*1024 + 1 bytes → assert `Read` returns an error wrapped with `*websocket.CloseError` whose `Code == 1009` (StatusMessageTooBig per RFC 6455 / PRD §11 SEC-6). Use `websocket.CloseStatus(err)` to extract the code portably.
