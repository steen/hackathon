...
- **What to assert:** Audit #78 dropped the *broadcast* of inbound frames, not the inbound read or the size check, so the body-cap close path is alive and externally observable. `apps/server/internal/wsapi/handler.go:247-249` (`if len(data) > MessageBodyLimit { conn.Close(StatusMessageTooBig, MessageBodyLimitCloseReason); return }`) closes any client that writes a frame larger than `MessageBodyLimit = 4 * 1024` (line 38) with code 1009 and reason `"message body exceeds 4 KiB limit"` (the `MessageBodyLimitCloseReason` constant on line 45). Primary assertion: dial `/ws`, write a single text frame of exactly 4096 bytes → does not trigger the body cap (the connection stays open from the body-cap perspective; the rate limiter still applies, so keep this to a single frame); dial fresh, write a single text frame of exactly 4097 bytes → assert `Read` returns an error wrapped with `*websocket.CloseError` whose `Code == 1009` and whose `Reason == "message body exceeds 4 KiB limit"` (use `websocket.CloseStatus(err)` for the code and read `(*websocket.CloseError).Reason` for the reason, identical to the AC-1 disambiguation pattern). Reason-text matches the exported `wsapi.MessageBodyLimitCloseReason` constant — the constant exists (lines 40-45) precisely so tests can assert it without a brittle substring match. Secondary check (optional, useful but not load-bearing): the same 4 KiB cap is also enforced on the REST send path, so `POST /api/channels/{id}/messages` with a 4097-char body returns a 4xx envelope and inserts no row; AC-3's literal target is the WS path, so prefer the WS assertion.
- **Layer:** Go (boot binary, raw WS).
- **File path:** `tests/e2e/phase-1/body-and-ws-caps/ws_message_body_cap_test.go`.
- **Setup it needs:** same as AC-1. The optional REST secondary check would also need `CHAT_DB_PATH=<tmpdir>/chatd.sqlite` so the channels/messages handlers are wired (see `main.go` line 109 conditional).
- **Helpers it can reuse:** harness from AC-1; `dialWS`, `closeStatus(err) int`. The optional REST secondary check would also reuse `register`, `login`, `createChannel`, `sendMessage`.
