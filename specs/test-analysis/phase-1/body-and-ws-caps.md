...
- **Wiring is real.** `apps/server/main.go:72` wraps the mux with `httpx.BodyCap`. This is the same wiring shape `phase-1/file-perms-and-headers` AC-2/AC-3 are still missing (`SecurityHeaders` not applied). Worth flagging because the pattern now exists in the codebase — the next implementer can compose `SecurityHeaders(httpx.BodyCap(mux))` cleanly (SecurityHeaders outermost so its `w.Header()` writes apply to BodyCap's 413 too).
...
2. Cross-feature observation worth surfacing: `httpx.BodyCap(mux)` wiring at `main.go:72` is the same pattern `feature-file-perms-and-headers` needs for `SecurityHeaders`. PR #37 already flagged that gap; it should now be a one-line follow-up to compose `SecurityHeaders(httpx.BodyCap(mux))` (order matters — `SecurityHeaders` has to be outermost: it sets `w.Header()` before calling `next.ServeHTTP`, so when `BodyCap` returns 413 those headers get flushed with the response. The opposite order — `BodyCap(SecurityHeaders(mux))` — would short-circuit before `SecurityHeaders` ever runs and the 413 would carry no headers.).
