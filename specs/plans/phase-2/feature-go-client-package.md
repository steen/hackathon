...
## Acceptance criteria
- A reusable Go module `packages/go-client` exposes typed methods for: `Login`, `Register`, `Me`, `Logout`, `ListChannels`, `CreateChannel`, `ListMessages`, `PostMessage`, `WsTicket`, and `Watch` (returns a stream of inbound events).
- The client handles base URL, auth token storage (in memory), and JSON/error-envelope decoding.
- The client is consumable from `apps/cli` via a Go workspace replace directive.

## Implementation steps
1. Create `packages/go-client/go.mod` and `client.go` with `New(baseURL, opts...)` returning a `*Client`.
