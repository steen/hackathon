...
2. Implement subcommands using a small dispatcher (e.g., `cobra` or stdlib `flag`-based router):
   - `login`, `channels`, `history`, `watch`, `send`.
3. Implement config persistence: load on startup; write on `login`; clear on `logout`.
4. Implement reconnect loop in `watch` with capped exponential backoff.
5. Update `scripts/smoke.sh` to use the new auth-aware commands.
