...
### AC-1 — Single root `go.mod` declares module `hackathon`; imports under `hackathon/<path>` build cleanly

- **Name:** `TestAC1_RootGoModDeclaresHackathonModule`
  - **Target file:** `tests/scaffold/go_mod_test.go`
  - **Asserts:**
    - `go.mod` exists at repo root
    - parsed `module` directive equals `hackathon`
    - no `go.work` file exists at repo root
    - no `apps/*/go.mod` or `packages/*/go.mod` file exists
...
|----|---------------------------|
| AC-1 | CI runs `go build ./...` and `go test ./...` from the repo root (single-module layout means a single command covers every package) |
