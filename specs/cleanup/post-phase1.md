...
## Implementation deviations (recorded post-phase-A)

> DEVIATION: B.4 — apps/server/internal/wsapi/handler.go post-merge body is ~45 lines (Handler) + 22 lines (readLoop), under the 80-line trigger. No extraction.
> DEVIATION: B.6 — apps/server/main.go: PR #45 already split `main()` into `func run() error` so `main()` is 5 lines. `run()` is ~110 lines but the wiring block (auth + ratelimit + channels) is one straight sequence with one outer `if`. Extracting to a helper would mostly forward arguments. Not extracting.
> DEVIATION: wsapi/handler.go:134 keeps `_ = userID` after the channels-and-messages merge. The original comment promised "userID will land on the per-connection state once channels-and-messages merges" — but the hub still has no per-conn user metadata; binding userID requires a hub-level change that is feature work, not structural cleanup. The redemption itself (the SEC-12 requirement) happens correctly. Recorded as a follow-up: wire userID into per-connection presence when the presence feature ships.
> DEVIATION: A.5 — golangci-lint not runnable locally in this sandbox (installer requires curl|sh which the harness blocks). CI will run v2.12.1 against `.golangci.yml`; any failures must be fixed in a follow-up commit on this branch before merge. ESLint + Prettier were runnable and surfaced two preexisting nits in `tests/smoke-test/wiring.test.ts` that I fixed inside the same #45 merge commit per CLAUDE.md "no preexisting cop-outs".
> DEVIATION: R3 (CHAT_ALLOWED_ORIGINS) — verified `internal/config.Config` has no `AllowedOrigins` field; main.go reads the env var directly via `os.Getenv(allowedOriginsEnv)` and runs it through `parseAllowedOrigins`. Matches what #39 already did; no new config field added.
