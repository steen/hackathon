...
//
// AC-2 already pins the four SEC-10 headers on 200/401/404. AC-3 layers
// failure-mode coverage on top: the contract is that the SecurityHeaders
// middleware is the outermost wrap, so even when inner middleware
// (BodyCap, ServeMux's MethodNotAllowed branch, or a handler returning a
// 4xx envelope) writes the response, the outer wrap has already populated
// the headers.
//
