// Vitest globalSetup for the phase-8 channel-ws-drift canary. Mirrors
// tests/e2e/phase-2/ts-api-client-package/globalSetup.ts: builds the Go
// server, picks a free port, spawns the binary with random per-suite
// secrets, polls readiness, and exposes E2E_SERVER_URL +
// E2E_INVITE_CODE via process.env. Additionally pre-builds the
// wsobserver helper binary (cmd/wsobserver) so the per-test child
// process spawn is just a fork+exec, not a fresh `go build`.
...
export default async function setup(): Promise<() => Promise<void>> {
  workDir = mkdtempSync(join(tmpdir(), "channel-ws-drift-e2e-"));
  const serverBin = join(workDir, "chat-server");
  const buildServer = spawnSync("go", ["build", "-o", serverBin, "./apps/server"], {
    cwd: repoRoot,
    stdio: "inherit",
  });
  ...
  const observerBin = join(workDir, "wsobserver");
  const buildObserver = spawnSync(
    "go",
    ["build", "-o", observerBin, "./tests/e2e/phase-8/channel-ws-drift/cmd/wsobserver"],
    { cwd: repoRoot, stdio: "inherit" },
  );
  if (buildObserver.status !== 0) {
    throw new Error("go build wsobserver failed");
  }
  if (!existsSync(observerBin)) {
    throw new Error(`wsobserver binary missing at ${observerBin}`);
  }
  ...
  process.env.E2E_WSOBSERVER_BIN = observerBin;
